...
const sheetOptions = [
  ...
  { length: 120, label: '22 × 120 in', price: 70 },
  { length: 150, label: '22 × 150 in', price: 90 },
  { length: 200, label: '22 × 200 in', price: 115 },
]
...
function getGangSheet(length: number) {
  ...
  return {
    length: billedLength,
    label: `22 × ${billedLength} in`,
    price,
    breakdown: fullSheets > 0 && remainderSheet ? `${fullSheets} × 200 in + ${remainderSheet.length} in` : fullSheets > 0 ? `${fullSheets} × 200 in` : remainderSheet?.label ?? '22 × 12 in',
  }
}
