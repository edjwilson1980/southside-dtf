...
function cropMarkPreviewCopy(markCount: number) {
  ...
  return `The ${countLabel} black 5 mm circles ARE printed on the PNG for the cutter camera, in the margins — not on the designs. A small black arrow points at the starting crop mark. The cut file sends the camera to those circles first so it can lock on before any knife path. Overlay crop marks are red circles. Left and right marks are 21.5 in apart. Under 12 in uses two pairs; 12–30 in uses three, spaced on that sheet length. Longer jobs repeat that pattern every 30 in. Red rectangles are the image plus 2 mm cut lines.`
}
